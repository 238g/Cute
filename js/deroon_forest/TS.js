...
function draw(start, end, color, size, compositeOperation, save) {
  ...
  context.scale(canvas.width/1140,canvas.height/heightSetting);
  context.strokeStyle = color;
...
function move(e) {
  ...
  sendMouse = {
    ...
    y: (heightSetting/canvas.height)*mouse.y
  };
  sendLastMouse = {
    x: (1140/canvas.width)*lastMouse.x,
    y: (heightSetting/canvas.height)*lastMouse.y
  };
...
function convertTouch(touch) {
  return {
    ...
    y: (heightSetting / canvas.height) * (touch.pageY - canvas.offsetTop),
...
$(document).ready(function () {
  ...
  $("#color-dialog").on("change", function(){
     // console.log($('#color-dialog').val());
    setColor($('#color-dialog').val());
    changeMouse();
  });
});
...
/////////////////////////////////////////////////////////////////////////////////////
function TogetherStart () {
  $('#StartBtn').hide();
  TogetherJS(this);
  return false;
}
TogetherJS.on("ready", startCompFunc);
function startCompFunc () {
  $('#StartBtn').hide();
  setTimeout(fixLanguage, 1000);
}
// TODO
function fixLanguage () {
  $('#togetherjs-walkthrough header').html('ようこそ！でろーんの森へ！');
}
